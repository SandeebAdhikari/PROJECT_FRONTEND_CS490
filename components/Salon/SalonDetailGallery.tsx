  ...
  return (
    <div className="mt-5 ">
      <div className="flex items-center justify-between mb-6">
        ...
        <span className="text-xs text-muted-foreground border border-border rounded-full px-3 py-1 font-inter">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 font-inter">
          {gallery.map((image, index) => (
            ...
            >
              ...
              />
              {image.title && (
                <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-white to-transparent text-foreground p-3">
                  <h3 className="text-sm font-semibold">{image.title}</h3>
                  {image.subtitle && (
                    <p className="text-xs text-foreground">{image.subtitle}</p>
                  )}
                </div>
              )}
            </div>
