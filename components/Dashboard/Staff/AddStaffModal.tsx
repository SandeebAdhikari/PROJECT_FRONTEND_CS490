...
const AddStaffModal: React.FC<AddStaffModalProps> = ({
  ...
  useEffect(() => {
    const loadRoles = async () => {
      if (!salonId || salonId === 0) {
        console.log("No salon ID, skipping role load");
        return;
      }
      try {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        setMessage(
          `✅ Staff added successfully! Staff Code: ${data.staff_code}. They can use this code to set up their PIN and log in.`
        );
        onAdded?.();
        setForm({
          first_name: "",
        ...
        {message && (
          <p
            className={`mt-3 text-sm ${
              message.toLowerCase().includes("added") || message.toLowerCase().includes("success")
