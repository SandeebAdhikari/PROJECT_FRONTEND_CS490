...
export default function StaffCard({ s }: { s: StaffMember }) {
    return (
        <div
            className="mt-4 border border-border bg-secondary rounded-lg p-6 hover:shadow-soft-br transition-smooth relative overflow-hidden"
            style={{
                backgroundImage:
                    "radial-gradient(900px 300px at 85% 0%, rgba(16,185,129,0.06), transparent)",
            }}
        >
           
            <div className="absolute right-4 top-4">
                <span
                    className={`px-3 py-1.5 rounded-full text-sm font-medium border border-transparent shadow-sm ${s.active
            ...
            <div className="flex items-start gap-4">
                {s.avatarUrl ? (
                    <Image src={s.avatarUrl} alt={s.name} width={48} height={48} className="rounded-full" />
                ) : (
                    <div className="h-12 w-12 rounded-full bg-white/70 flex items-center justify-center text-sm font-semibold text-foreground/80">
                        {s.name.split(" ").map((n) => n[0]).slice(0, 2).join("").toUpperCase()}
                    </div>
                )}

                <div className="flex-1">
                    <h3 className="font-sans text-lg sm:text-xl font-extrabold">{s.name}</h3>
            ...
            <div className="mt-6 grid grid-cols-1 gap-5 sm:grid-cols-2">
                <div className="text-base">
                    ...
                    <div className="mt-1 text-[16px] sm:text-lg font-medium">{s.email}</div>

                    <div className="mt-5 text-muted-foreground font-inter">Specialties:</div>
                ...
                <div className="text-base">
                    ...
                    <div className="mt-1 text-[16px] sm:text-lg font-medium">{s.phone}</div>
                </div>
            </div>
            ...
            <div className="mt-4 flex items-center justify-between">
                ...
                <div className="flex items-center gap-3 text-muted-foreground">
                    <button title="View" className="rounded p-1 hover:text-foreground transition-smooth">
                        <Eye className="h-5 w-5" />
                    </button>
                    <button title="Edit" className="rounded p-1 hover:text-foreground transition-smooth">
